...
Going to try other shit for database setup.
"""


def before_request():
    g.database = DatabaseManager()
...
class DatabaseManager:
    ...
    def open_db(self):
        if "database" not in g:
            ...
            g.database.row_factory = sqlite.Row
        return g.database
    ...
    def insert_conversation(
        self, username: str, model: str, title: str, prompt: str, prompt_text: str
    ):
        cursor = self.conn.cursor()
        cursor.execute(
            """
    ...
    def update_conversation_summaries(
        self, conversation_id, long_summary: str, short_summary: str
    ):
        self.conn.execute(
            """
    ...
    def get_conversation_summaries(self, conversation_id):
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT summary, short_summary FROM conversations WHERE id=?",
            (conversation_id,),
        )
        return cursor.fetchone()
    ...
    def get_conversations_for_user(self, username: str) -> list:
        cursor = self.conn.cursor()
        cursor.execute("SELECT * FROM conversations WHERE username=?", (username,))
        rows = cursor.fetchall()
        columns = [col[0] for col in cursor.description]
        return [dict(zip(columns, row)) for row in rows]
    ...
    def get_user_info(self) -> list:
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT username, sats, admin, creation_date, last_login FROM users"
        )
        rows = cursor.fetchall()
        columns = [col[0] for col in cursor.description]
    ...
    def get_conversation_history(self, conversation_id) -> list:
        cursor = self.conn.cursor()
        cursor.execute(
    ...
    def insert_message(self, conversation_id, role: str, content: str) -> int:
        cursor = self.conn.cursor()
        cursor.execute(
    ...
    def delete_oldest_message(self, conversation_id) -> str | None:
        cursor = self.conn.cursor()
        cursor.execute(
    ...
    def delete_message(self, conversation_id, message_id) -> str | None:
        cursor = self.conn.cursor()
        cursor.execute(
            "DELETE FROM messages WHERE conversation_id=? AND id=?",
    ...
    def delete_conversation_history(self, conversation_id, message_id) -> str | None:
        cursor = self.conn.cursor()
        cursor.execute(
            "DELETE FROM conversation_history WHERE conversation_id=? AND id=?",
            (conversation_id, message_id),
        )
        self.conn.commit()
    ...
    def get_payment(self, payment_hash: str) -> dict | None:
        cursor = self.conn.cursor()
    ...
    def update_payment(self, payment_hash: str, field: str, value):
        self.conn.execute(
            f"UPDATE payments SET {field}=? WHERE payment_hash=?", (value, payment_hash)
        )
        self.conn.commit()
    ...
    def get_invoice_status(self, payment_hash: str):
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT invoice_status FROM payments WHERE payment_hash=?", (payment_hash,)
        )
