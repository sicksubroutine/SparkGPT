...
class DataUtils:
  ...
  def check_old_markdown() -> None:
    ...
    This method checks if the 'static/markdown/' directory exists. If it doesn't, the directory is created.
    Then, it iterates through each file in the directory and removes any file with the '.md' extension.
    """
  ...
  def clean_up_invoices() -> None:
    ...
    This method iterates through each file in the 'static/qr/' directory and removes any file with the '.png' extension.
    """
    path = "static/qr/"
    for filename in os.listdir(path):
  ...
  def summary_of_messages(convo: str) -> tuple:
    ...
    Notes:
      - The method retrieves the messages from a database based on the given conversation identifier.
      - It iterates through the messages and collects the user's content until reaching the second message.
      - The user's messages are concatenated to generate a summary.
      - A prompt is created with a specific format for summarization.
      - The user's summary messages are included in the conversation along with the system prompt.
      - The openai_response method from the ChatUtils class is used to obtain a response from the OpenAI assistant.
      - The original longer response is preserved before any modifications.
      - The concise response is generated by removing punctuation and joining the response words with underscores.
    """
    d_base = g.d_base
  ...
  def export_as_markdown(convo:str, title:str, model:str) -> str:
    ...
    Notes:
      - The method retrieves the messages from a database based on the given conversation identifier.
      - It iterates through the messages and creates a Markdown file with the following format:
...
class ChatUtils:
  ...
  def openai_response(messages: list, model:str="gpt-3.5-turbo") -> tuple:
    ...
    Args:
      ...
      model (str): The OpenAI model to use for generating the response. (default: "gpt-3.5-turbo")
    ...
    Raises:
      ...
      openai.error.APIConnectionError: If a connection error occurs while communicating with the OpenAI API.
      openai.error.RateLimitError: If the API request exceeds the rate limit.

    Notes:
      - The method uses exponential backoff for retries in case of API errors or connection failures.
  ...
  def estimate_tokens(text:str, method:str="max") -> int|None:
    ...
    Notes:
      - The 'max' method returns the maximum of the estimated tokens based on word count and character count.
...
class BitcoinUtils:
  ...
  def get_bitcoin_cost(tokens: int, model:str="gpt-3.5-turbo") -> int:
    ...
    Notes:
      ...
      - The cost is calculated using the current Bitcoin price obtained from the Kraken API.
